...
export function Modal({
  ...
  // Focus + scroll lock run only when `open` flips — not on every render. Callers
  // pass an inline onOpenChange (new ref each render); keeping it out of these deps
  // stops the initial focus from being reapplied and stealing focus mid-typing.
  React.useEffect(() => {
    if (!open) return
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 backdrop-blur-xs p-4 sm:items-center animate-overlay-in"
    ...
    >
      <div
        ...
        className={cn(
          "flex w-[540px] max-w-full flex-col overflow-hidden rounded-xl bg-[var(--ds-background-100)] text-[var(--ds-gray-1000)] shadow-[var(--ds-shadow-modal)] max-h-[min(800px,80vh)] animate-modal-in",
          className
        )}
      >
        {(title || description) && (
